...
function isPalindrome(str) {
  ...
  const newStr1 = newStr.replace(/\s/g, '');
  const reversed = newStr1.split('').reverse().join('');
  if (newStr1 !== reversed) {
    return false;
  }
  return true;
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    const rotatedCharCode = ((char.charCodeAt(0) - shift + 13) % 26) + shift;
...
function getCardId(value) {
  ...
  return index !== -1 ? index : 'Card not found in the initial deck';
}
